...
#  Streamlit UI
# ══════════════════════════════════════════════════════════════════
st.set_page_config(page_title="테라로사 외부몰 주문취합", page_icon="☕", layout="wide")
...
with st.sidebar:
    ...
    with st.expander("➕ 새 세트 상품 추가", expanded=False):
        new_set_name = st.text_input("세트 상품명", placeholder="예: 드립백&유리머그 세트",
                                      key="new_set_name")
        ...
        for i, (sname, comps) in enumerate(st.session_state.set_config.items()):
            with cols[i % 3]:
                ...
                for c in comps:
                    ...
                    st.caption(f"× {c['qty']}  {c['name']}{weight_str}")
    ...
    if st.button("🚀 취합 파일 생성", type="primary", use_container_width=True):
        with st.spinner("처리 중..."):
            try:
                ...
                result_bytes = process(orders_bytes, codes_bytes, st.session_state.set_config)
                ...
                for ch, info in report["channels"].items():
                    rows_val.append({"채널": ch, "원본": info["orig"], "취합": info["result"],
                                     "일치 여부": "✅" if info["match"] else "❌"})
                st.dataframe(pd.DataFrame(rows_val).set_index("채널"), use_container_width=True)
...
| **세트 분리** | 사이드바에서 설정한 세트 상품을 구성 품목별로 분리 (하늘색 강조) |
